...
function App() {
        ...
        if (!localStorage.getItem('userInfo')) {
            ...
            , JSON.stringify([
                ...
                { userName:'브이클로'
                    ...
                    , userBirth:'1940-12-31'
                    , userSmsAccept:'Y'
                    , userEmail:'aaaa@naver.com'
                    , userEmailAccept:'Y'
                    , userReferrerId:'admin'
                },
            ])//JSON.stringify
        )}//if
        if (!localStorage.getItem('cartItems')) {
            ...
            , JSON.stringify([
                { id: 101
                    ...
                    , price: 22800
                    , count: 1
                    , img: 'https://images.unsplash.com/photo-1576185055363-6d7c88000919?q=80&w=200' 
                }
  ...
  const {isOtp, myOtp} = otpInfo;
    ...
    const onLoginSubmit = (id, password) => { //로그인 (아이디, 패스워드) submit
        ...
        if(joinCheck) {
            const loginCheck = ({
                isLogin: true
                , id: joinCheck.userId
                , password: joinCheck.userPassword
            });
    ...
    const onPhoneSubmit = (phone) => { // 휴대전화 인증
        ...
        if(phoneCheck) {
            const newOtp = String(Math.floor(Math.random() * 1000000) + 1000000)
                .substring(1, 7);
    ...
    const onJoinSubmit = (name, id, password, phone, birth, smsAccept, email, emailAccept, referrerId) => {
        ...
        const error = [
            {check: nameCheck, rsn:'이름: 한글로만 입력하세요'}
            , {check: idCheck, rsn:'아이디는 영문과 숫자만 사용가능 5글자 이상 입력해야합니다'}
            , {check: passwordCheck, rsn: '패스워드: 영문으로 입력하시고, 숫자 1글자 이상을 포함 바랍니다'}
            , {check: phoneCheck, rsn:'잘못된 전화번호 형식입니다.'}
            , {check: !joinCheck, rsn:'이미 회원가입된 아이디, 전화번호 입니다.'}
            , {check: emailCheck, rsn:'잘못된 이메일 형식입니다.'}
        ...
        if(!joinCheck && passwordCheck && emailCheck && referrerIdCheck) {
            join.push({userName: name
                        , userId: id
                        , userPassword: password
                        , userPhone: phone
                        , userBirth: birth
                        , userSmsAccept: smsAccept
                        , userEmail: email
                        , userEmailAccept: emailAccept
                        , userReferrerId: referrerId
            });//join.push
    ...
    const selectedProductTotal = cartItems.filter(item => selectedIds.includes(item.id))
    ...
    return (
        <>
            ...
            <Routes>
                ...
                <Route path="*" element={
                    <div className='app-container'>
                        ...
                        <Section 
                                ...
                                selectedProductTotal={selectedProductTotal} 
                ...
                <Route path="/login/find-id" element={
                    <FindId 
                ...
                <Route path="/login/find-password" element={
                    <FindPassword 
                            onPhoneSubmit={onPhoneSubmit} 
                            onOtpSubmit={onOtpSubmit} 
                            otpInfo={otpInfo} 
                ...
                <Route path="/payMent" element={
                    loginInfo.isLogin ? 
                                        <PayMent finalTotal={finalTotal}/> : <Navigate to="/login"/> 
                }/>
            </Routes>
        </>
    );
